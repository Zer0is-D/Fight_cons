...
namespace FightCons
{
    ...
    internal class Store
    {
        ...
        private List<ItemChar> WeaponList = new List<ItemChar>();
        private List<ItemChar> ArmorList = new List<ItemChar>();
        Dictionary<string, List<string>> WeaponsByMaterial = new Dictionary<string, List<string>>();
        Dictionary<string, List<string>> ArmorByMaterial = new Dictionary<string, List<string>>();
        ...
        public void GoodsOut(Hero hero, List<ItemChar> itemChars)
        {
            ...
            if (itemChars[0].ItemType == ItemTypes.Weapon)
                Console.Write($"# {ItemStats(hero.CharacterWeapon)}\n");
            else
                Console.Write($"# {ItemStats(hero.CharacterArmor)}\n");

            Output.WriteColorLine(ConsoleColor.Yellow, $"# Монеты: {hero.Money}", $"{Output.MoneySymbol}\n");
            ...
            switch (chois)  
            {
                case 0:
                    ...
                    break;
                case 1:
                    if (Output.Spent(hero, Output.ShowNewItemsCost, "", "Ну не за бесплатно же!"))
                    {
